...
def battery(output):
    if output.endswith('%'):
        ...
        icon = icons[next(filter(lambda r: percent in r, icons.keys()))]
...
left = [
    ...
    widget.GroupBox(
        ...
        padding=7,
        fmt=groupBox,
        padding_y=6,  # Add vertical padding for block-like look
        # background =theme['alt_background'],
    ), space,

    widget.CPU(
        format='󰍛  {load_percent:2.0f}%',
        update_interval=4,
        mouse_callbacks={
            'Button1': lazy.spawn(terminal + ' -e btop --force-utf'),
        },
        # background = theme['alt_background'],
    ), space,

    widget.Memory(
        # back.ground=theme['alt_background'],
        format='󰍜  {MemUsed:2.0f}GB',
        measure_mem='G',
        update_interval=4,
        mouse_callbacks={
            'Button1': lazy.spawn(terminal + ' -e btop --force-utf'),
